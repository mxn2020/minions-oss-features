...
description: Agent skills for working with Minions Oss-features MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-oss-features data.
---

# Minions Oss-features Agent Skills

Skills for agents operating on the `minions-oss-features` toolbox.
...
```bash
# TypeScript
pnpm add @minions-oss-features/sdk
...
The `oss-features` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-oss-features/sdk/schemas';
...
```python
from minions_oss_features.schemas import custom_types
...
- This agent only writes to `minions-oss-features` — it reads from other toolboxes but never writes to them
